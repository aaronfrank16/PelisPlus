...
public class SeriesJpaController implements Serializable {
    ...
    public Series findSeriesByProduct(Productos id) {
        List<Series> serie = null;
        EntityManager em = getEntityManager();
        System.out.println("Buscado serie po idProducto "+id);
        Query consulta = em.createNamedQuery("Series.findByIdProducto");
        consulta.setParameter("idProducto", id);
        serie = consulta.getResultList();
        em.close();
        if (!serie.isEmpty()) {
