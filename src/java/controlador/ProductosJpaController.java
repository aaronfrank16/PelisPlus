...
public class ProductosJpaController implements Serializable {
    ...
    public void create(Productos productos) throws RollbackFailureException, Exception {
        ...
        if (productos.getCarritoProductoList() == null) {
            productos.setCarritoProductoList(new ArrayList<CarritoProducto>());
        }
        ...
        try {
            ...
            for (CarritoProducto carritoProductoListCarritoProductoToAttach : productos.getCarritoProductoList()) {
            ...
            productos.setCarritoProductoList(attachedCarritoProductoList);
            ...
            for (CarritoProducto carritoProductoListCarritoProducto : productos.getCarritoProductoList()) {
                Productos oldIdProductoOfCarritoProductoListCarritoProducto = carritoProductoListCarritoProducto.getIdProducto();
                carritoProductoListCarritoProducto.setIdProducto(productos);
                carritoProductoListCarritoProducto = em.merge(carritoProductoListCarritoProducto);
                if (oldIdProductoOfCarritoProductoListCarritoProducto != null) {
                    oldIdProductoOfCarritoProductoListCarritoProducto.getCarritoProductoList().remove(carritoProductoListCarritoProducto);
                    oldIdProductoOfCarritoProductoListCarritoProducto = em.merge(oldIdProductoOfCarritoProductoListCarritoProducto);
                }
            }
    ...
    public void edit(Productos productos) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<CarritoProducto> carritoProductoListOld = persistentProductos.getCarritoProductoList();
            List<CarritoProducto> carritoProductoListNew = productos.getCarritoProductoList();
            ...
            for (CarritoProducto carritoProductoListOldCarritoProducto : carritoProductoListOld) {
                if (!carritoProductoListNew.contains(carritoProductoListOldCarritoProducto)) {
                    ...
                    illegalOrphanMessages.add("You must retain CarritoProducto " + carritoProductoListOldCarritoProducto + " since its idProducto field is not nullable.");
                }
            }
            ...
            productos.setCarritoProductoList(carritoProductoListNew);
            ...
            for (CarritoProducto carritoProductoListNewCarritoProducto : carritoProductoListNew) {
                if (!carritoProductoListOld.contains(carritoProductoListNewCarritoProducto)) {
                    Productos oldIdProductoOfCarritoProductoListNewCarritoProducto = carritoProductoListNewCarritoProducto.getIdProducto();
                    carritoProductoListNewCarritoProducto.setIdProducto(productos);
                    carritoProductoListNewCarritoProducto = em.merge(carritoProductoListNewCarritoProducto);
                    if (oldIdProductoOfCarritoProductoListNewCarritoProducto != null && !oldIdProductoOfCarritoProductoListNewCarritoProducto.equals(productos)) {
                        oldIdProductoOfCarritoProductoListNewCarritoProducto.getCarritoProductoList().remove(carritoProductoListNewCarritoProducto);
                        oldIdProductoOfCarritoProductoListNewCarritoProducto = em.merge(oldIdProductoOfCarritoProductoListNewCarritoProducto);
                    }
                }
            }
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<CarritoProducto> carritoProductoListOrphanCheck = productos.getCarritoProductoList();
            for (CarritoProducto carritoProductoListOrphanCheckCarritoProducto : carritoProductoListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Productos (" + productos + ") cannot be destroyed since the CarritoProducto " + carritoProductoListOrphanCheckCarritoProducto + " in its carritoProductoList field has a non-nullable idProducto field.");
            }
    ...
    public List<Productos> getCategorias(int id) {
        List<Productos> producto;
        EntityManager em = getEntityManager();
        System.out.println("Buscando peliculas " + id);
        Query consulta = em.createNamedQuery("Productos.findByTipo");
        consulta.setParameter("tipo", 1);
        producto = consulta.getResultList();
        for (int i = 0; i < producto.size(); i++) {
            if (producto.get(i).getIdCategoria().getIdCategoria() != id) {
                producto.remove(i);
                i--;
            }
        }
    ...
    public List<Productos> getCategoriasSerie(int id) {
        List<Productos> producto;
        EntityManager em = getEntityManager();
        System.out.println("Buscando series " + id);
        Query consulta = em.createNamedQuery("Productos.findByTipo");
    ...
    public List<Productos> getMejoresPelis() {
        List<Productos> producto;
        EntityManager em = getEntityManager();
        System.out.println("Buscando mejores peliculas");
        Query consulta = em.createNamedQuery("Productos.findByTipo");
        consulta.setParameter("tipo", 1);
        producto = consulta.getResultList();
        for (int i = 0; i < producto.size(); i++) {
            if (producto.get(i).getRating() < 9) {
                producto.remove(i);
                i--;
            }
        }
    ...
    public List<Productos> getMejoresSeries() {
        List<Productos> producto;
        EntityManager em = getEntityManager();
        System.out.println("Buscando mejores serie");
        Query consulta = em.createNamedQuery("Productos.findByTipo");
        consulta.setParameter("tipo", 2);
        producto = consulta.getResultList();
        for (int i = 0; i < producto.size(); i++) {
